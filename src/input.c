...
//   int move_which = 0;
	...
	if (options.key_or_joy == 1)
	{
		...
		if (player.swing_hold == 1
		        && joy[0].button[options.joy_button[player.blade_index]].b == 0)
			player.swing_hold = 0;

		if (player.ring_hold == 1
		        && joy[0].button[options.joy_button[player.ring_index]].b == 0)
			player.ring_hold = 0;
...
// NO!!! Need to calc angle then move in that angle
	...
	else
	{
		...
		if (player.swing_hold == 1
		        && !key[player.key[CKEY_FIRE1 + player.blade_index]])
			player.swing_hold = 0;

		if (player.ring_hold == 1
		        && !key[player.key[CKEY_FIRE1 + player.ring_index]])
			player.ring_hold = 0;

		if (key[player.key[CKEY_FIRE1]]
		        || key[player.key[CKEY_FIRE2]] || key[player.key[CKEY_FIRE3]])
			player.firing = 1;
...
// see swbeam function in pbullet.c for damage
	...
	case WPN_SEEKER:
		...
		player.wfired = m;
#ifdef SHOW_DAMAGE
		calc_damage (pbullet[0].dam * (3 + player.wlevel[m]),
		             player.recycle_speed[m]);
// see lwbeam function in pbullet.c for damage
#endif
...
void fire_seekers (int m)
{
	...
	int angles[13] =
	    { ANGLE_2, ANGLE_2 + ANGLE_16, ANGLE_2 - ANGLE_16, ANGLE_2 + ANGLE_8,
	      ANGLE_2 - ANGLE_8,
	      ANGLE_2 + ANGLE_8 + ANGLE_16, ANGLE_2 - ANGLE_8 - ANGLE_16,
	      ANGLE_4, -ANGLE_4, ANGLE_4 - ANGLE_8, -ANGLE_4 + ANGLE_8,
	      ANGLE_4 - ANGLE_16, -ANGLE_4 + ANGLE_16
	    };
...
//     quick_cloud(CLOUD_CIRCLE3, xa, ya, xpart(angle, 2500), ypart(angle, 2500), 35, 8, 970);
	...
	case WPN_CANNON:
		...
		for (i = 0; i < 4; i++)
		{
			quick_cloud (CLOUD_CIRCLE2, xa - 1000 * i, ya,
			             -1000 - i * 1000 - level * 100, i * 400 + level * 30,
			             20 + i * 4 + level, TCOL_ORANGE, 920);
			quick_cloud (CLOUD_CIRCLE2, xa + 1000 * i, ya,
			             1000 + i * 1000 + level * 100, i * 400 + level * 30,
			             20 + i * 4 + level, TCOL_ORANGE, 920);
		}
	...
	case WPN_RECT:
		...
		{
			angle += ANGLE_4;
			quick_cloud (CLOUD_CIRCLE5, xa + xpart (angle, 10000),
			             ya + ypart (angle, 10000), xpart (angle, 3000),
			             ypart (angle, 3000), 60, TCOL_ORANGE, 920);
			quick_cloud (CLOUD_CIRCLE5, xa + xpart (angle, 10000),
			             ya + ypart (angle, 10000), xpart (angle, 3000),
			             ypart (angle, 3000), 40, TCOL_YELLOW, 920);
		}
		break;
	case WPN_MGUN:
		xa = pbullet[b].x;
		ya = pbullet[b].y + player.wlevel[m] * 1000;
		quick_cloud (CLOUD_CIRCLE2, xa, ya, 0, -1500, 15 + level / 4, TCOL_YELLOW,
		             930);
		quick_cloud (CLOUD_CIRCLE3, xa, ya, 0, -1500, 30 + level / 2, TCOL_ORANGE,
		             930);
		for (i = 0; i < 4; i++)
		{
			quick_cloud (CLOUD_CIRCLE2, xa, ya, 0 - grand (1000) + grand (1000),
			             (i * -1500) - grand (2000), 10 + grand (10) + level,
			             TCOL_ORANGE, 930);
		}
		break;
	case WPN_SCATTER:
		xa = pbullet[b].x;
		ya = pbullet[b].y;
		quick_cloud (CLOUD_CIRCLE1, xa, ya - 7000, 0, -2000, 10 + level / 3,
		             TCOL_YELLOW, 960);
		quick_cloud (CLOUD_CIRCLE2, xa, ya - 7000, 0, -2000, 20 + level / 1,
		             TCOL_ORANGE, 960);
		/*    angle = grand(ANGLE_1);
		    for (i = 0; i < 6; i ++)
		    {
