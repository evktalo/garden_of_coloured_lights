...
void game_loop(void)
{
 ...
 if (arena.shift_message > 0)
 {
        ...
        && arena.phase > 0)
  {
   ...
   switch(arena.level)
   {
    case 1: arena.target_palette = arena.phase; break;
//     play_effectwfvx(WAV_STAGE1, tone [NOTE_1C + arena.phase], 200, 320);
     break;
    case 2: arena.target_palette = BASECOL_L2_1 + arena.phase; break;
//     play_effectwfvx(WAV_STAGE2, tone [NOTE_1C + arena.phase], 200, 320);
     break;
    case 3: arena.target_palette = BASECOL_L3_1 + arena.phase; break;
//     play_effectwfvx(WAV_STAGE3, tone [NOTE_1C + arena.phase], 200, 320);
//     break;
    case 4: arena.target_palette = BASECOL_L4_1 + arena.phase; break;
//     play_effectwfvx(WAV_STAGE4_2, tone [NOTE_1C + arena.phase], 200, 320);
//     break;
   }
   arena.change_palette = 0;
  }
/*  switch(arena.level)
  {
   case 2:
    if (arena.phase > 0 && arena.shift_message == 150)
     play_effectwfvx(WAV_STAGE2, tone [NOTE_2C + arena.phase], 160, 320);
    break;
   case 3:
    if (arena.phase > 0 && arena.shift_message == 250)
     play_effectwfvx(WAV_STAGE3, tone [NOTE_1C + arena.phase], 250, 500);
    break;
   case 4:
    if (arena.phase > 0 && arena.shift_message == 150)
...
void run_player(void)
{
    ...
    char cycling = (player.recycle [0] > 0) + (player.recycle [1] > 0) + (player.recycle [2] > 0);

    if (player.blade_index != -1
        && (player.swing_state != BLADE_LEFT && player.swing_state != BLADE_RIGHT))
        {
         cycling ++;
         firing_fast ++;
        }
...
//      continue;
        ...
        && (player.wtype [i] != SLOW_WEAPON || firing_fast == 0))
     {
      ...
      if (cycling > 0) // probably always true, but div by zero sucks
       cycle /= cycling;
...
/*    if (player.recycle > 0)
    {
     cycle =
     player.recycle -= player.recycle_speed / cycling;
     if (player.recycle <= 0)
     {
      player.recycle = 0;
      player.wfired = -1;
...
/*    if (player.wfired != -1)
    {
     player.wframe += player.wframe_speed;
     if (player.wframe_speed > 0 && player.wframe >= player.wframe_max)
     {
      player.wframe = player.wframe_max;
      switch
     }
...
//     player.wx [0] = player.x + xpart(player.orbital_angle, 12000);
//     player.wy [0] = player.y + ypart(player.orbital_angle, 12000);
     ...
     for (i = 0; i < 3; i ++)
     {
      ...
      if (player.weapon [i] == WPN_SWBEAM)
      {
       ...
       if (player.swbeam_counter [i] > 0)
       {
         ...
         quick_cloud(CLOUD_CIRCLE5, player.wx [i] + grand(4000) - grand(4000), player.wy [i] - 5000 + grand(4000) - grand(4000),  grand(1000) - grand(1000), grand(1000) - grand(1000), grand(20) + player.swbeam_counter [i] / 2, TCOL_ORANGE, 960);
       }
      }
     }
...
{
/*
       player.wx [m] = player.x + xpart(orb_angle, 30000);
       player.wy [m] = player.y + ypart(orb_angle, 30000);
       orb_angle += j;
//       player.swing_angle = radians_to_angle(atan2(player.y - player.wy [i], player.x - player.wx [i]));
*/
...
//         player.recycle = 1;
         ...
         player.swing_angle -= BLADE_SPEED;
         player.swing_angle &= 1023;
//         if (player.swing_angle <= ANGLE_2 - ANGLE_16 && player.swing_angle > ANGLE_4)
...
/*
        case BLADE_LEFT:
         player.wx [m] = player.x - 20000;
         player.wy [m] = player.y - 10000;
         player.swing_speed_x = 0;
         player.swing_speed_y = 0;
         break;
        case BLADE_RIGHT:
...
/*
        case BLADE_CW_SLASH:
         target_x = player.x + 20000;
         target_y = player.y - 10000;
         goal_angle = radians_to_angle(atan2(target_y - player.wy [m], target_x - player.wx [m]));
         player.swing_speed_x += xpart(goal_angle, 3000);
         player.swing_speed_y += ypart(goal_angle, 3000);

         goal_angle = radians_to_angle(atan2(player.x - player.wy [m], player.y - player.wx [m]));
         player.swing_speed_x += xpart(goal_angle, 1800);
         player.swing_speed_y += ypart(goal_angle, 1800);

         if (player.wx [m] > target_x - 5000
             && player.wx [m] < target_x + 5000
             && player.wy [m] > target_y - 5000
             && player.wy [m] < target_y + 5000)
              end_swing = 1;

         if (player.wx [m] > target_x - 15000
             && player.wx [m] < target_x + 15000
             && player.wy [m] > target_y - 15000
             && player.wy [m] < target_y + 15000)
             {
              drag_blade();
             }
...
/*       if (player.swing_state > BLADE_RIGHT)
       {
        player.wx [m] += player.swing_speed_x;
        player.wy [m] += player.swing_speed_y;
        player.recycle = 1;
       }*/
...
//        player.swing_x [player.swing_pos] = player.wx [m] + xpart(player.swing_angle, 1000);
//        player.swing_y [player.swing_pos] = player.wy [m] + ypart(player.swing_angle, 1000);
...
/* quick_cloud(CLOUD_CIRCLE2, player.swing_x [player.swing_pos] + xpart(bangle, 16000), player.swing_y [player.swing_pos] + ypart(bangle, 16000), 0, 0, 10, TCOL_ORANGE, 0);
 bangle += ANGLE_2;
 quick_cloud(CLOUD_CIRCLE2, player.swing_x [player.swing_pos] + xpart(bangle, 16000), player.swing_y [player.swing_pos] + ypart(bangle, 16000), 0, 0, 10, TCOL_ORANGE, 0);*/
...
void drag_blade(void)
{
 player.swing_speed_x *= 800;
 player.swing_speed_x /= 1000;
 player.swing_speed_y *= 800;
...
void player_hit(void)
{
 ...
 for (i = 0; i < 30; i ++)
 {
  ...
  quick_cloud(CLOUD_CIRCLE2, x, y, xpart(angle, speed), ypart(angle, speed), 20 + grand(20), TCOL_ORANGE, 910);
  angle = grand(ANGLE_1);
  speed = 5000 + grand(10000);
  quick_cloud(CLOUD_CIRCLE3, x, y, xpart(angle, speed), ypart(angle, speed), 20 + grand(20), TCOL_YELLOW, 870);
 }
...
//  player.weapon [i] = MULTI_BASIC;
  ...
  if (options.tourist == 0)
   player.wlevel [i] = 0;
     ...
     player.wlevel [i] = options.tourist; //(options.tourist - 1) * 2 - 1;
  player.wstatus [i] = 0;
  player.wpixels [i] = 0;
  player.wgained [i] = 0;
  player.recycle [i] = 0;
...
// arena.phase_bonus = 0;
 ...
 switch(arena.level)
 {
  ...
  case 3:
   ...
   arena.next_phase [4] = -1;
/*   arena.next_phase [0] = 1000;
   arena.next_phase [1] = 1900;
   arena.next_phase [2] = 2800;
   arena.next_phase [3] = 3600;
   arena.next_phase [4] = 4500;
   arena.next_phase [5] = -1;*/
   break;
  case 4:
...
void init_stage_3(void)
{
 ...
 int j;
  ...
  for (j = 0; j < 14; j ++)
  {
/*     arena.part_x1 [i] [j] = 5 + grand(10);
     arena.part_y1 [i] [j] = 5 + grand(10);
     arena.part_x2 [i] [j] = 5 + grand(10);
...
//      tile = ISLAND1_EMPTY; // need to fix this
      ...
      if ((u == ISLAND1_EMPTY || ur == ISLAND1_EMPTY)
       && d != ISLAND1_EMPTY
       && (l == ISLAND1_EMPTY || dl == ISLAND1_EMPTY)
       && r != ISLAND1_EMPTY)
       {
      ...
      if (u != ISLAND1_EMPTY
       && (d == ISLAND1_EMPTY || dr == ISLAND1_EMPTY)
      ...
      if (u != ISLAND1_EMPTY
       && (d == ISLAND1_EMPTY || dl == ISLAND1_EMPTY)
       && l != ISLAND1_EMPTY
