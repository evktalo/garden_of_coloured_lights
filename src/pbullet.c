...
//            pbullet[b].x += pbullet[b].x_speed;
    ...
    if (pbullet[b].persist == 0)
    {
     if (pbullet[b].x <= X_MIN - 20000
      ...
      || pbullet[b].y <= Y_MIN - 20000
      || pbullet[b].y >= Y_MAX + 20000)
      {
       destroy_pbullet(b);
       continue;
      }
   ...
   if (x >= enemy[e].x - eclass[enemy[e].type].xsize + eclass[enemy[e].type].xoffset
       ...
       && y >= enemy[e].y - eclass[enemy[e].type].ysize + eclass[enemy[e].type].yoffset)
       {
        ...
        if (max_y > y)
         max_y = y;
         max_y -= grand(16000);
         quick_cloud(CLOUD_CIRCLE1, x, max_y, grand(2000) - grand(2000), grand(2000) - grand(2000) - 500 - player.wlevel [m] * 250, 3 + grand(3) + grand(player.wlevel [m]), TCOL_YELLOW, 0);
         max_y -= grand(8000);
         quick_cloud(CLOUD_CIRCLE1, x, max_y, grand(2000) - grand(2000), grand(2000) - grand(2000) - 500 - player.wlevel [m] * 250, 7 + grand(5) + player.wlevel [m], TCOL_ORANGE, 0);
...
// int destroy_type = 0;
 ...
 for (e = 0; e < NO_ENEMIES; e ++)
 {
  if (enemy[e].type == ENEMY_NONE)
   continue;
  if (pbullet[b].x >= enemy[e].x - eclass[enemy[e].type].xsize + eclass[enemy[e].type].xoffset - pbullet[b].xsize
      && pbullet[b].x <= enemy[e].x + eclass[enemy[e].type].xsize + eclass[enemy[e].type].xoffset + pbullet[b].xsize
      && pbullet[b].y >= enemy[e].y - eclass[enemy[e].type].ysize + eclass[enemy[e].type].yoffset - pbullet[b].ysize
      && pbullet[b].y <= enemy[e].y + eclass[enemy[e].type].ysize + eclass[enemy[e].type].yoffset + pbullet[b].ysize)
      {
       {
...
void pbullet_explode(int b, int hit)
{
 ...
 switch(pbullet[b].type)
 {
  ...
  case PBULLET_RING:
   ...
   quick_cloud(CLOUD_PRING, pbullet[b].x, ya, 0, 0, pbullet[b].status, 0, 0);
   break;
  case PBULLET_MGUN:
