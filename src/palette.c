...
void run_palette_shift(void)
{
 ...
 for (i = 5; i < 14; i ++)
 {
   for (j = 0; j < 3; j ++)
   {
    if (current_base [i - 5] [j] < base_colours [arena.target_palette] [i - 5] [j])
    {
     current_base [i - 5] [j] ++;
     changed = 1;
    }
...
void create_inner_bullet_colour(int pa, int index, int r, int g, int b)
{
 ...
 for (i = 0; i < 13; i ++)
 {
  palet [pa] [26 + 26 * index + i + 1].r = limit_colour(palet [pa] [i + 1].r / 1.5 + r * 1.5);
  palet [pa] [26 + 26 * index + i + 1].g = limit_colour(palet [pa] [i + 1].g / 1.5 + g * 1.5);
  palet [pa] [26 + 26 * index + i + 1].b = limit_colour(palet [pa] [i + 1].b / 1.5 + b * 1.5);
