...
void draw_stopper1(int x, int y, int front_down, int fx, int fy, int mx, int my, int bx, int by);
...
void draw_dart(int x, int y, int angle, int out, int timeout, int cord_col, int col);
void draw_diamond(int x, int y, int angle, int out1, int out2, int cord_colour);
...
void run_display(int show)
{
  ...
  if (show == 1)
  {
   switch(arena.level)
   {
    ...
    case 2: draw_background2(); break;
    case 3: draw_background3(); break;
    case 4: draw_background4(); break;
   }
   draw_enemies();
  }
  if (player.in_play == 1)
...
//  textprintf_ex(display, font, 10, 10, COL_WHITE, -1, "%i fps", frames_per_second);
 ...
 if (arena.level_finished > 0 && arena.level_finished < 140)
 {
  ...
  if (arena.level_finished < 120 && arena.level_finished > 20)
   ...
   {
    case 1:
     textprintf_centre_ex(display, font, 320, 180, -1, -1, "STAGE  ONE  COMPLETE");
     break;
    case 2:
     textprintf_centre_ex(display, font, 320, 180, -1, -1, "STAGE  TWO  COMPLETE");
     break;
    case 3:
     textprintf_centre_ex(display, font, 320, 180, -1, -1, "STAGE  THREE  COMPLETE");
     break;
    case 4:
     textprintf_centre_ex(display, font, 320, 180, -1, -1, "FINAL  STAGE  COMPLETE");
     break;
   }
...
//  TRANS_MODE
  ...
  rectfill(display, 320 - x, 188 - y, 320 + x, 188 + y, TRANS_YELLOW_IN - 1);
  ...
  if (arena.game_over < 180)
   textprintf_centre_ex(display, font, 320, 180, -1, -1, "G A M E      O V E R");
 }
...
*/
  ...
  if (arena.new_level_sign < 100 && arena.new_level_sign > 20)
   ...
   {
       case 1:
        textprintf_centre_ex(display, font, 320, 180, -1, -1, "S T A G E   O N E"); break;
       case 2:
        textprintf_centre_ex(display, font, 320, 180, -1, -1, "S T A G E   T W O"); break;
       case 3:
        textprintf_centre_ex(display, font, 320, 180, -1, -1, "S T A G E   T H R E E"); break;
       case 4:
        textprintf_centre_ex(display, font, 320, 180, -1, -1, "F I N A L   S T A G E"); break;
   }
 }
...
int check_pixel(int x, int y)
{
  ...
  if ((pix >= TRANS_B1_IN && pix < TRANS_B2_OUT)
   ...
   || (pix >= TRANS_B4_IN && pix < TRANS_B5_OUT)
   || (pix >= TRANS_B5_IN))
    return 1;
...
/*     xb = ANGLE_2;
  ...
  switch(player.weapon [i])
  {
   case WPN_CANNON:
    ...
    draw_rle_sprite(display, RLE_multi [WPN_CANNON] [k].sprite, xa - RLE_multi [WPN_CANNON] [k].x, ya - RLE_multi [WPN_CANNON] [k].y);
    break;
   case WPN_MGUN:
//    if (player.wfired == i)
    {
    ...
    draw_rle_sprite(display, RLE_multi [WPN_MGUN] [k].sprite, xa - RLE_multi [WPN_MGUN] [k].x, ya - RLE_multi [WPN_MGUN] [k].y - 3);
    break;
   case WPN_SCATTER:
//    if (player.wfired == i)
    {
    ...
    draw_rle_sprite(display, RLE_multi [WPN_SCATTER] [k].sprite, xa - RLE_multi [WPN_SCATTER] [k].x, ya - RLE_multi [WPN_SCATTER] [k].y);
    break;
   case WPN_SEEKER:
//    if (player.wfired == i)
    {
    ...
    draw_rle_sprite(display, RLE_multi [WPN_SEEKER] [k].sprite, xa - RLE_multi [WPN_SEEKER] [k].x, ya - RLE_multi [WPN_SEEKER] [k].y);
    break;
   case WPN_RECT:
//    if (player.wfired == i)
    {
    ...
    draw_rle_sprite(display, RLE_multi [WPN_RECT] [k].sprite, xa - RLE_multi [WPN_RECT] [k].x, ya - RLE_multi [WPN_RECT] [k].y);
    break;
   case WPN_LWBEAM:
/*    if (player.wfired == i)
    {
    ...
    if (player.beam_open [i] > 7)
     k = 2;
    draw_rle_sprite(display, RLE_multi [WPN_LWBEAM] [k].sprite, xa - RLE_multi [WPN_LWBEAM] [k].x, ya - RLE_multi [WPN_LWBEAM] [k].y);
    break;
   case WPN_SWBEAM:
//    if (player.wfired == i)
    {
    ...
    draw_rle_sprite(display, RLE_multi [WPN_SWBEAM] [k].sprite, xa - RLE_multi [WPN_SWBEAM] [k].x, ya - RLE_multi [WPN_SWBEAM] [k].y);
    break;
   case WPN_EIGHT:
    draw_rle_sprite(display, RLE_multi [WPN_EIGHT] [k].sprite, xa - RLE_multi [WPN_EIGHT] [k].x, ya - RLE_multi [WPN_EIGHT] [k].y);
    break;
   default:
   ...
   if (player.swing_state > BLADE_RIGHT)
    draw_rle_sprite(display, RLE_multi [WPN_BLADE] [1].sprite, xa - 3, ya - 3);
     else
      draw_rle_sprite(display, RLE_multi [WPN_BLADE] [0].sprite, xa - 4, ya - 4);
   xb = RLE_blade [player.swing_sprite_angle].x - xpart(player.swing_sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 14);
   yb = RLE_blade [player.swing_sprite_angle].y - ypart(player.swing_sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 14);
   draw_trans_rle_sprite(display, RLE_blade [player.swing_sprite_angle].sprite, xa - xb, ya - yb);
   j = player.swing_pos;
   k = 0;
   TRANS_MODE
   do
   {
    ...
    if (j == player.swing_pos || k >= player.swing_length)
     break;
    pline(display, old_x, old_y, player.swing_x [j] / GRAIN, player.swing_y[j] / GRAIN, TRANS_YELLOW_OUT);
    xa = (player.swing_length - k);
    xa /= player.swing_strength [j];
    xb = player.wlevel [player.blade_index];
//    xa *= 10 + xb;
//    xa /= 10;
    fcircle(player.swing_x [j] / GRAIN, player.swing_y [j] / GRAIN, xa, TCOL_YELLOW);
    if (xb > 3)
    {
     xa *= 10 + xb;
     xa /= 10;
     fcircle(player.swing_x [j] / GRAIN, player.swing_y [j] / GRAIN, xa, TCOL_ORANGE);
    }
   } while (TRUE);
   END_TRANS_MODE
   break; //  end blade
...
// print_number(100,100,player.swing_hold);
 ...
 if (player.beam [i] != -1 || player.beam_flicker [i] > 0)
 {
  ...
  if (xb > 15)
   ...
   draw_trans_rle_sprite(display, RLE_lwbeamshock [xb].sprite, xa - yb, ya - yb - 3);
...
//         || (player.wlevel [i] > 0 && player.beam_flicker [i] > 0))
     if (player.wlevel [i] > 0 && (player.beam_fade [i] > 5 || player.beam_flicker [i] > 0))
     {
      TRANS_MODE
      vline(display, xa, ya, 0, TRANS_ORANGE_IN);
      vline(display, xa - 1, ya, 0, TRANS_ORANGE_OUT);
...
#define SPIN_SPEED 16
 ...
 if (player.grace > 0)
 {
  ...
  fcircle(xa + xpart(arena.counter * SPIN_SPEED, yb), ya + ypart(arena.counter * SPIN_SPEED, yb), xb, TCOL_ORANGE);
  fcircle(xa + xpart(arena.counter * SPIN_SPEED + ANGLE_4, yb), ya + ypart(arena.counter * SPIN_SPEED + ANGLE_4, yb), xb, TCOL_ORANGE);
  fcircle(xa + xpart(arena.counter * SPIN_SPEED - ANGLE_4, yb), ya + ypart(arena.counter * SPIN_SPEED - ANGLE_4, yb), xb, TCOL_ORANGE);
  fcircle(xa + xpart(arena.counter * SPIN_SPEED - ANGLE_2, yb), ya + ypart(arena.counter * SPIN_SPEED - ANGLE_2, yb), xb, TCOL_ORANGE);

//  yb = 25 + xpart(ANGLE_1 - ((arena.counter * 32) & 1023) + ANGLE_6, 13);
  yb = 25 - xpart(arena.counter * 64, 8);
  fcircle(xa + xpart(ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), ya + ypart(ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), xb, TCOL_YELLOW);
  fcircle(xa + xpart(ANGLE_4 + ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), ya + ypart(ANGLE_4 + ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), xb, TCOL_YELLOW);
  fcircle(xa + xpart(ANGLE_2 + ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), ya + ypart(ANGLE_2 + ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), xb, TCOL_YELLOW);
  fcircle(xa + xpart(- ANGLE_4 + ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), ya + ypart(- ANGLE_4 + ANGLE_1 - ((arena.counter * SPIN_SPEED) & 1023), yb), xb, TCOL_YELLOW);
/*  xb *= 15;
  xb /= 10;
...
//  print_number(10, 180, damage_per_time);
 ...
 {
  rectfill(display, 635 - i * 10 - 2, 30, 635 - i * 10 - 10, 38, COL_WHITE);
 ...
 for (i = 0; i < 3; i ++)
 {
  ...
  if (player.rpixels [i] > 0)
  {
   ...
   {
     rect(display, 5, ya + 3, 5 + player.rpixels [i], ya + 11, TRANS_YELLOW_OUT);
     rectfill(display, 6, ya + 4, 4 + player.rpixels [i], ya + 10, TRANS_YELLOW_IN);
   }
    else
    {
     rect(display, 5, ya + 3, 5 + player.rpixels [i], ya + 11, TRANS_ORANGE_OUT);
     rectfill(display, 6, ya + 4, 4 + player.rpixels [i], ya + 10, TRANS_ORANGE_IN);
    }
  }
  END_TRANS_MODE
  xa = 2;
  ...
  if (player.wlevel [i] < 9)
  {
   ...
   {
    rect(display, 5, ya + 3, 5 + player.wpixels [i] + 1, ya + 9, TRANS_ORANGE_OUT);
    rectfill(display, 6, ya + 4, 5 + player.wpixels [i] + 0, ya + 8, TRANS_ORANGE_IN);
   }
   END_TRANS_MODE
  }
  TRANS_MODE
  ...
  {
   rect(display, 4 + k * 5, ya + 13, 7 + k * 5, ya + 18, TRANS_YELLOW_OUT);
   rectfill(display, 5 + k * 5, ya + 14, 6 + k * 5, ya + 17, TRANS_YELLOW_IN);
  }
  END_TRANS_MODE
 }
...
//  if (boss.hp > 0)
  if (line_x > 0)
  {
   rectfill(display, 320 - line_x + 1, 10, 320 + line_x - 1, 15, TRANS_YELLOW_IN);
...
void draw_an_enemy(int e)
{
    ...
    switch(enemy[e].type)
    {
      ...
      case ENEMY_BOSS2:
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS2_CORE], x - 27, y - 14);

        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS2_IN_L], x - 85 - boss.out_sides, y - 47);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS2_IN_R], x + 16 + boss.out_sides, y - 47);

        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS2_OUT_L], x - 104 - boss.out_sides - boss.out_sides2, y - 2);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS2_OUT_R], x + 73 + boss.out_sides + boss.out_sides2, y - 2);
        ...
        circlefill(display, x - 53 - boss.out_sides + xpart(boss.t_angle [0], 5), y + 40 + ypart(boss.t_angle [0], 5), 2, COL_COL1);
        circle(display, x - 53 - boss.out_sides + xpart(boss.t_angle [0], 5), y + 40 + ypart(boss.t_angle [0], 5), 2, COL_OUTLINE);
        ...
        circlefill(display, x - 32 - boss.out_sides + xpart(boss.t_angle [3], 5), y - 20 + ypart(boss.t_angle [3], 5), 2, COL_COL1);
        circle(display, x - 32 - boss.out_sides + xpart(boss.t_angle [3], 5), y - 20 + ypart(boss.t_angle [3], 5), 2, COL_OUTLINE);

        circlefill(display, x + 53 + boss.out_sides, y + 40, 10, COL_COL4);
        circle(display, x + 53 + boss.out_sides, y + 40, 10, COL_OUTLINE);
        circlefill(display, x + 53 + boss.out_sides, y + 40, 7, COL_COL1);
        circle(display, x + 53 + boss.out_sides, y + 40, 7, COL_OUTLINE);
        circlefill(display, x + 53 + boss.out_sides + xpart(boss.t_angle [2], 5), y + 40 + ypart(boss.t_angle [2], 5), 2, COL_COL1);
        circle(display, x + 53 + boss.out_sides + xpart(boss.t_angle [2], 5), y + 40 + ypart(boss.t_angle [2], 5), 2, COL_OUTLINE);

        circlefill(display, x + 32 + boss.out_sides, y - 20, 10, COL_COL4);
        circle(display, x + 32 + boss.out_sides, y - 20, 10, COL_OUTLINE);
        circlefill(display, x + 32 + boss.out_sides, y - 20, 7, COL_COL1);
        circle(display, x + 32 + boss.out_sides, y - 20, 7, COL_OUTLINE);
        circlefill(display, x + 32 + boss.out_sides + xpart(boss.t_angle [1], 5), y - 20 + ypart(boss.t_angle [1], 5), 2, COL_COL1);
        circle(display, x + 32 + boss.out_sides + xpart(boss.t_angle [1], 5), y - 20 + ypart(boss.t_angle [1], 5), 2, COL_OUTLINE);

        break;

      case ENEMY_BOSS1:

        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_CORE], x - 77, y - 47);

        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_OUT_L], x - 103 - boss.out_sides2, y - 32);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_OUT_R], x + 46 + boss.out_sides2, y - 32);

        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_UL], x - 101 - boss.out_sides2, y - 76 - boss.out_out_up);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_UR], x + boss.out_sides2, y - 76 - boss.out_out_up);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_DL], x - 101 - boss.out_sides2, y + 13 + boss.out_out_up);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_DR], x + boss.out_sides2, y + 13 + boss.out_out_up);

        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_U], x - 34, y - 47 - boss.out_up);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_D], x - 34, y + 17 + boss.out_up);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_L], x - 48 - boss.out_sides, y - 33);
        draw_rle_sprite(display, eRLE_large1 [L_ENEMY_BOSS1_R], x + 18 + boss.out_sides, y - 33);
...
//        circle(display, x - 71 - boss.out_sides2 + xpart(boss.angle2, 11), y + ypart(boss.angle2, 11), 3, COL_OUTLINE);
        circlefill(display, x - 71 - boss.out_sides2 + xpart(boss.t_angle [0], 11), y + ypart(boss.t_angle [0], 11), 3, COL_COL1);
        circle(display, x - 71 - boss.out_sides2 + xpart(boss.t_angle [0], 11), y + ypart(boss.t_angle [0], 11), 3, COL_OUTLINE);
        ...
        circlefill(display, x + 71 + boss.out_sides2, y, 16, COL_COL1);
        circle(display, x + 71 + boss.out_sides2, y, 16, COL_OUTLINE);
        circlefill(display, x + 71 + boss.out_sides2 + xpart(boss.t_angle [1], 11), y + ypart(boss.t_angle [1], 11), 3, COL_COL1);
        circle(display, x + 71 + boss.out_sides2 + xpart(boss.t_angle [1], 11), y + ypart(boss.t_angle [1], 11), 3, COL_OUTLINE);
...
L_ENEMY_BOSS1_OUT_R,*/
      ...
      case ENEMY_BEAMER:
        ...
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_CORE], x - 19, y - 18);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_L], x - 39 - xa, y - 14);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_R], x + 5 + xa, y - 14);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_U], x - 11, y - 26 - 4 + xa);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_D], x - 9, y + 14 + 4 - xa);
        circlefill(display, x, y, 11, COL_COL3);
        circle(display, x, y, 11, COL_OUTLINE);
        circlefill(display, x + xpart(enemy[e].angle1, 10), y + ypart(enemy[e].angle1, 10), 3, COL_COL3);
        circle(display, x + xpart(enemy[e].angle1, 10), y + ypart(enemy[e].angle1, 10), 3, COL_OUTLINE);
       break;
      ...
      case ENEMY_SHOOTER:
        ...
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_CORE], x - 19, y - 18);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SHOOTER_L], x - 39 - xa, y - 14);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SHOOTER_R], x + 5 + xa, y - 14);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SHOOTER_U], x - 11, y - 26 - 4 + xa);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BEAMER_D], x - 9, y + 14 + 4 - xa);
        circlefill(display, x, y, 11, COL_COL3);
        circle(display, x, y, 11, COL_OUTLINE);
        circlefill(display, x + xpart(enemy[e].angle1, 10), y + ypart(enemy[e].angle1, 10), 3, COL_COL3);
      ...
      case ENEMY_DRIFTER1:
        xa = enemy[e].status / 4;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DRIFTER1_CORE], x - 19, y - 15);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DRIFTER1_L], x - 25 - xa, y - 2 + xa);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DRIFTER1_R], x + 6 + xa, y - 2 + xa);
        break;
      case ENEMY_DRIFTER2:
        xa = enemy[e].status / 4;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DRIFTER2_CORE], x - 19, y - 15);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DRIFTER2_L], x - 25 - xa, y - 2 + xa);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DRIFTER2_R], x + 10 + xa, y - 2 + xa);
        break;
      case ENEMY_ESCORT:
        xa = enemy[e].status / 4;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_ESCORT_CORE], x - 20, y - 14);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_ESCORT_L], x - 22 - xa, y - 14);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_ESCORT_R], x + 7 + xa, y - 14);
        break;
      case ENEMY_RISER:
        xa = 0;
        if (enemy[e].phase == 1) // opening
        ...
        if (xa < 0)
         xa = 0;
//        xa = enemy[e].status / 4;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_RISER1_L], x - 18 - xa, y - 8);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_RISER1_R], x + 8 + xa, y - 8);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_RISER1_CORE], x - 13, y - 8);
        break;

      case ENEMY_SMALL2:
        xa = enemy[e].pulse1;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SMALL_CORE2], x - 7, y - 9);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SMALL_OUTER_L], x - 14 - xa, y - 16);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SMALL_OUTER_R], x - 0 + xa, y - 16);
        for (i = 0; i < 3; i ++)
        {
...
//            circle(display, x + xb, y + yb, 3, COL_WHITE);
      ...
      case ENEMY_LARGER:
        xa = enemy[e].pulse1;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_LARGER_CORE1], x - 10, y - 10);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_LARGER_OUTER_L], x - 17 - xa, y - 23);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_LARGER_OUTER_R], x - 0 + xa, y - 23);
        for (i = 0; i < 4; i ++)
        {
            ya = enemy[e].petal_angle [i] / (ANGLE_1 / SMALL_ROTATIONS);
            ya &= SMALL_ROTATIONS - 1;
...
//        pline(display, enemy[e].x_target [0] / 1000, enemy[e].y_target [0] / 1000, enemy[e].x_target [1] / 1000, enemy[e].y_target [1] / 1000, COL_COL1);
      ...
      case ENEMY_LARGER2:
        xa = enemy[e].pulse1;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_LARGER_CORE2], x - 10, y - 10);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_LARGER_OUTER_L], x - 17 - xa, y - 23);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_LARGER_OUTER_R], x - 0 + xa, y - 23);
        for (i = 0; i < 3; i ++)
        {
      ...
      case ENEMY_WINGS:
        xa = enemy[e].pulse1;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_WINGS_CORE1], x - 11, y - 20);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_WINGS_OUTER_L], x - 21 - xa, y - 26);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_WINGS_OUTER_R], x - 0 + xa, y - 26);
        for (i = 0; i < 2; i ++)
        {
            ya = enemy[e].petal_angle [i] / (ANGLE_1 / SMALL_ROTATIONS);
            ya &= SMALL_ROTATIONS - 1;
      ...
      case ENEMY_WINGS2:
        xa = enemy[e].pulse1;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_WINGS_CORE2], x - 11, y - 20);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_WINGS_OUTER_L], x - 21 - xa, y - 26);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_WINGS_OUTER_R], x - 0 + xa, y - 26);
        for (i = 0; i < 6; i ++)
        {
      ...
      case ENEMY_MEGA:
        ...
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_MEGA_OUTER_L], x - 38 - xa, y - 44);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_MEGA_OUTER_R], x - 0 + xa, y - 44);
        for (i = 0; i < 4; i ++)
        {
            ya = enemy[e].petal_angle [i] / (ANGLE_1 / SMALL_ROTATIONS);
            ya &= SMALL_ROTATIONS - 1;
      ...
      case ENEMY_BOSS3:
        xa = enemy[e].pulse1;
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_BOSS3_CORE], x - 21, y - 35);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_MEGA_OUTER_L], x - 38 - xa, y - 44);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_MEGA_OUTER_R], x - 0 + xa, y - 44);
        for (i = 0; i < 9; i ++)
        {
      ...
      case ENEMY_CRUISER1:
       ...
       circlefill(display, x + xpart(enemy[e].angle, 10), y + ypart(enemy[e].angle, 10), 3, COL_COL3);
       circle(display, x + xpart(enemy[e].angle, 10), y + ypart(enemy[e].angle, 10), 3, COL_OUTLINE);
       ...
       circlefill(display, x + xpart(enemy[e].angle1, 6), y - 41 + ypart(enemy[e].angle1, 6), 2, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1, 6), y - 41 + ypart(enemy[e].angle1, 6), 2, COL_OUTLINE);
       break;
      ...
      case ENEMY_CRUISER2:
       ...
       circlefill(display, x + xpart(enemy[e].angle1, 8), y + ypart(enemy[e].angle1, 8), 3, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1, 8), y + ypart(enemy[e].angle1, 8), 3, COL_OUTLINE);
      ...
      case ENEMY_CRUISER3:
       ...
       circlefill(display, x + xpart(enemy[e].angle1, 19), y + ypart(enemy[e].angle1, 19), 4, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1, 19), y + ypart(enemy[e].angle1, 19), 4, COL_OUTLINE);
      ...
      case ENEMY_CRUISER4:
       ...
       circlefill(display, x + xpart(enemy[e].angle1, 17), y + ypart(enemy[e].angle1, 17), 4, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1, 17), y + ypart(enemy[e].angle1, 17), 4, COL_OUTLINE);
      ...
      case ENEMY_POD1:
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_POD1_CORE], x - 32, y - 58);
         ...
         if (enemy[e].phase == 1) // opening
          draw_pod1(x, y, (100 - enemy[e].target_time) / 3, (100 - enemy[e].target_time) / 2, e);
...
//       circle(display, x + xpart(enemy[e].angle1, 18), y + ypart(enemy[e].angle1, 18), 3, COL_OUTLINE);
       ...
       circlefill(display, x + xpart(enemy[e].angle1 + ANGLE_4 - ANGLE_16, 18), y + ypart(enemy[e].angle1 + ANGLE_4 - ANGLE_16, 18), 2, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1 + ANGLE_4 - ANGLE_16, 18), y + ypart(enemy[e].angle1 + ANGLE_4 - ANGLE_16, 18), 2, COL_OUTLINE);
       circlefill(display, x + xpart(enemy[e].angle1 - ANGLE_4 + ANGLE_16, 18), y + ypart(enemy[e].angle1 - ANGLE_4 + ANGLE_16, 18), 2, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1 - ANGLE_4 + ANGLE_16, 18), y + ypart(enemy[e].angle1 - ANGLE_4 + ANGLE_16, 18), 2, COL_OUTLINE);

       circlefill(display, x + xpart(enemy[e].angle1 + ANGLE_4 + ANGLE_8, 18), y + ypart(enemy[e].angle1 + ANGLE_4 + ANGLE_8, 18), 2, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1 + ANGLE_4 + ANGLE_8, 18), y + ypart(enemy[e].angle1 + ANGLE_4 + ANGLE_8, 18), 2, COL_OUTLINE);
       circlefill(display, x + xpart(enemy[e].angle1 - ANGLE_4 - ANGLE_8, 18), y + ypart(enemy[e].angle1 - ANGLE_4 - ANGLE_8, 18), 2, COL_COL3);
       circle(display, x + xpart(enemy[e].angle1 - ANGLE_4 - ANGLE_8, 18), y + ypart(enemy[e].angle1 - ANGLE_4 - ANGLE_8, 18), 2, COL_OUTLINE);
       break;
      case ENEMY_HOOK:
       ...
       if (xa < 0)
        xa = 0;
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_HOOK1 + xa], x - 16, y - 17);
//        pline(display, x, y, enemy[e].x_target [0] / 1000, enemy[e].y_target [0] / 1000, COL_WHITE);
//        pline(display, enemy[e].x_target [0] / 1000, enemy[e].y_target [0] / 1000, enemy[e].x_target [1] / 1000, enemy[e].y_target [1] / 1000, COL_COL1);
       break;
      case ENEMY_HOOK2:
       ...
       if (xa < 0)
        xa = 0;
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_HOOK2_1 + xa], x - 17, y - 17);
       break;
      case ENEMY_DROPPER:
       xa = enemy[e].pulse1;
       if (xa > 2)
        xa = 2;
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DROPPER3 - xa], x - 14, y - 23 + xa);
       break;
      case ENEMY_HUNTER:
       xa = 2 - enemy[e].pulse1;
       if (xa < 0)
        xa = 0;
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_HUNTER1 + xa], x - 28, y - 27 + xa);
       break;
      case ENEMY_RING:
       xa = 2 - enemy[e].pulse1;
       if (xa < 0)
        xa = 0;
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_RING1 + xa], x - 24 - xa, y - 24 - xa);
       break;
      case ENEMY_BOSS4:
...
/*        xa = ;//;(arena.counter / 4) % 8;
        ...
        {
            case 3:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER1_1], x - 15, y - 17);
             break;
            case 2:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER1_2], x - 16, y - 17);
             break;
            case 1:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER1_3], x - 17, y - 17);
             break;
            case 0:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER1_4], x - 18, y - 18);
             break;
        }
...
//        textprintf_ex(display, font, x, y - 40, COL_WHITE, -1, "%i", enemy[e].angle1);
      ...
      case ENEMY_SWOOPER2:
        ...
        {
            case 3:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER2_1], x - 20, y - 20);
             break;
            case 2:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER2_2], x - 21, y - 20);
             break;
            case 1:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER2_3], x - 22, y - 20);
             break;
            case 0:
             draw_rle_sprite(display, eRLE_small1 [S_ENEMY_SWOOPER2_4], x - 23, y - 20);
             break;
        }
        break;
      case ENEMY_DIVER1:
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_CENTRE], x - 17, y - 29);
       if (enemy[e].phase == 0)
       {
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_INNER_L], x - 18, y - 29);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_OUTER_L], x - 26, y - 23);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_INNER_R], x + 9, y - 29);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_OUTER_R], x + 18, y - 23);
        break;
       }
       if (enemy[e].phase == 5)
       {
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_INNER_L], x - 18, y - 29);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_OUTER_L], x - 26, y - 23 + 18);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_INNER_R], x + 9, y - 29);
        draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_OUTER_R], x + 18, y - 23 + 18);
        break;
       }
       if (enemy[e].phase == 1)
      ...
      case ENEMY_DARTER1:
         ...
         draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER1_CENTRE], x - 13, y - 18);
        ...
        circlefill(display, x + xpart(enemy[e].angle1, 5), y + ypart(enemy[e].angle1, 5), 2, COL_COL1);
        circle(display, x + xpart(enemy[e].angle1, 5), y + ypart(enemy[e].angle1, 5), 2, COL_OUTLINE);

    if (enemy[e].phase == 0
        || enemy[e].phase == 4
        || enemy[e].phase == 5)
         draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER1_MAIN], x - 24, y - 19);

//      textprintf_ex(display, font, x, y, COL_COL1, COL_COL4, "%i ", enemy[e].x);
//        pline(display, x, y, enemy[e].x_target [0] / 1000, enemy[e].y_target [0] / 1000, COL_WHITE);
        if (enemy[e].phase == 1)
         draw_darter1(x, y, 5 - (enemy[e].target_time / 6), 15 - (enemy[e].target_time / 2));
        if (enemy[e].phase == 2)
         draw_darter1(x, y, 5, 15);
        if (enemy[e].phase == 3)
         draw_darter1(x, y, (enemy[e].target_time / 6), (enemy[e].target_time / 2));
...
*/
      ...
      case ENEMY_WALKER:
       ...
       if (xa < 0)
        xa = 0;
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_WALKER1 + xa], x - 76, y - 70);
       break;
      case ENEMY_SPINNER:
       xa = 2 - enemy[e].pulse1;
       if (xa < 0)
        xa = 0;
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_SPINNER1 + xa], x - 52, y - 86 + xa);
       break;

      case ENEMY_DARTER2:
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_CORE], x - 12, y - 12);
    ...
    if (enemy[e].phase == 0
        ...
        if (enemy[e].phase == 1)
         draw_darter2(x, y, 5 - (enemy[e].target_time / 6), 15 - (enemy[e].target_time / 2));
        if (enemy[e].phase == 2)
         draw_darter2(x, y, 5, 15);
        if (enemy[e].phase == 3)
         draw_darter2(x, y, (enemy[e].target_time / 6), (enemy[e].target_time / 2));
       break;
      case ENEMY_CARRIER1_PART:
         ...
         circlefill(display, x + xpart(enemy[e].angle1, 4), y + ypart(enemy[e].angle1, 4), 1, COL_COL1);
         circle(display, x + xpart(enemy[e].angle1, 4), y + ypart(enemy[e].angle1, 4), 1, COL_OUTLINE);
        ...
        ya = enemy[e].pulse1 / 2;
        if (enemy[e].phase == 1)
         draw_carrier1_part(x, y, 3 - (enemy[e].target_time / 10) + xa, 6 - (enemy[e].target_time / 5) + ya);
        if (enemy[e].phase == 2)
         draw_carrier1_part(x, y, 3 + xa, 6 + ya);
        if (enemy[e].phase == 3)
         draw_carrier1_part(x, y, enemy[e].target_time / 10 + xa, enemy[e].target_time / 5 + ya);
...
//         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_CORE], x - 68, y - 85);
               draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_CORE], x - 69, y - 85);
         ...
         if (enemy[e].phase == 1) // opening
          draw_carrier1(x, y, (eclass[ENEMY_CARRIER1].speed3 - enemy[e].target_time) / 3);
         ...
         circlefill(display, x + xpart(angle, 22), y + ypart(angle, 22), 4, COL_COL1);
         circle(display, x + xpart(angle, 22), y + ypart(angle, 22), 4, COL_OUTLINE);
         angle += ANGLE_3;
         circlefill(display, x + xpart(angle, 22), y + ypart(angle, 22), 4, COL_COL1);
         circle(display, x + xpart(angle, 22), y + ypart(angle, 22), 4, COL_OUTLINE);
         angle += ANGLE_3 + 8;
...
//         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1], x - 46, y - 101);
         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_CENTRE], x - 45, y - 49);
...
void draw_pod1(int x, int y, int out, int up_out, int e)
{
   ...
   if (out1 > 3)
    out1 = 3;
       ...
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_POD1_BODYL], x - 55 - out1 - enemy[e].pulse1, y - 53);
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_POD1_BODYR], x + 3 + out1 + enemy[e].pulse1, y - 53);
       out1 = out - 25;
       if (out1 < 0)
        out1 = 0;
       if (up_out > 45)
        up_out = 45;
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_POD1_TOPL], x - 20 - out1 + enemy[e].pulse1 / 2, y - 55 - up_out);
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_POD1_TOPR], x - 1 + out1 - enemy[e].pulse1 / 2, y - 55 - up_out);
       out *= 0.7;
       draw_rle_sprite(display, eRLE_large1 [L_ENEMY_POD1_TOPC], x - 9, y - 56 - out);
}

void draw_stopper1(int x, int y, int front_down, int fx, int fy, int mx, int my, int bx, int by)
{

         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_FRONT], x - 21, y + 25 + front_down);
         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_F_L], x - 40 - fx, y + 8 + fy);
         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_M_L], x - 46 - mx, y - 75 + my);
         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_B_L], x - 21 - bx, y - 102 - by);

         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_F_R], x + 15 + fx, y + 8 + fy);
         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_M_R], x + 15 + mx, y - 75 + my);
         draw_rle_sprite(display, eRLE_large1 [L_ENEMY_STOPPER1_B_R], x + 0 + bx, y - 102 - by);
...
int save_out = out;
    ...
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_TOP1], x - 32, y - 85 - out);
    out *= (float) 0.8;
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_TOP2], x - 52, y - 74 - out);
    out *= (float) 0.5;
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_TOP3], x - 64, y - 54 - out);
    out = save_out;
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_BOTTOM2], x - 46, y + 37 + out);
    out *= (float) 0.8;
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_BOTTOM1], x - 62, y + 12 + out);

    out = save_out;
    int out2 = out / 3;

    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_MIDL1], x - 68 - out, y - 31 - out2);
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_MIDL2], x - 68 - out, y - 1 + out2);
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_MIDR1], x + 18 + out, y - 31 - out2);
    draw_rle_sprite(display, eRLE_large1 [L_ENEMY_CARRIER1_MIDR2], x + 20 + out, y - 1 + out2);
...
//         if (out < 3)
         ...
         draw_rle_sprite(display, eRLE_small1 [S_ENEMY_C1P1_TOP], x - 8, y - 10 - out);
         draw_rle_sprite(display, eRLE_small1 [S_ENEMY_C1P1_BOTTOM1], x - 11, y + 5 + out);
         draw_rle_sprite(display, eRLE_small1 [S_ENEMY_C1P1_BOTTOM2], x - 8, y + 10 + out2);
         draw_rle_sprite(display, eRLE_small1 [S_ENEMY_C1P1_MIDL], x - 12 - out2, y - 6);
...
void draw_darter2(int x, int y, int out2, int out1)
{
       ...
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_TOP], x - 6, y - 23 - out2);
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_BOTTOM], x - 6, y + 3 + out2);
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_OUTER_L], x - 33 - out2, y - 19 + out1);
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_INNER_L], x - 18 - out2, y - 19 - out1);
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_OUTER_R], x + 18 + out2, y - 19 + out1);
       draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DARTER2_INNER_R], x + 6 + out2, y - 19 - out1);
...
void draw_diver1(int x, int y, int xa, int ya)
{
 ...
 draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_INNER_L], x - 18 - xa, y - 29);
 draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_OUTER_L], x - 26 - xa, y - 23 + ya);
 draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_INNER_R], x + 9 + xa, y - 29);
 draw_rle_sprite(display, eRLE_small1 [S_ENEMY_DIVER1_OUTER_R], x + 18 + xa, y - 23 + ya);
...
void draw_an_ebullet(int b)
{
 ...
 switch(ebullet[b].type)
 {
     ...
     case EBULLET_SPIN:
      ...
      END_TRANS_MODE
      xa = ANGLE_1 / ebullet[b].y_speed;
      ya = ebullet[b].angle;
      for (i = 0; i < ebullet[b].y_speed; i ++)
      {
        fcircle(x + xpart(ya, ebullet[b].status), y + ypart(ya, ebullet[b].status), ebullet[b].x_speed, ebullet[b].colour);
        fcircle(x + xpart(ya, ebullet[b].status), y + ypart(ya, ebullet[b].status), ebullet[b].x_speed + 4, ebullet[b].accel);
        fcircle(x + xpart(ya + ANGLE_32, ebullet[b].status), y + ypart(ya + ANGLE_32, ebullet[b].status), ebullet[b].x_speed / 2, ebullet[b].accel);
        fcircle(x + xpart(ya - ANGLE_32, ebullet[b].status), y + ypart(ya - ANGLE_32, ebullet[b].status), ebullet[b].x_speed / 2, ebullet[b].accel);
        ya += xa;
      }
      break;
     case EBULLET_BEAM2:
       ...
       if (ebullet[b].angle2 > 100)
       {
        ...
        fcircle(ebullet[b].x_speed / GRAIN, ebullet[b].y_speed / GRAIN, ya + grand(4), TCOL_B5);
        fcircle(ebullet[b].x_speed / GRAIN - xpart(xc, 25), ebullet[b].y_speed / GRAIN - ypart(xc, 25), ya / 2 + grand(10), TCOL_B5);
        fcircle(ebullet[b].x_speed / GRAIN - xpart(xc, 35), ebullet[b].y_speed / GRAIN - ypart(xc, 35), ya / 3 + grand(4), TCOL_B5);
         break;
       }
      ...
      {
       fcircle(ebullet[b].x_speed / GRAIN, ebullet[b].y_speed / GRAIN, ebullet[b].angle2 + grand(4), TCOL_B5);
       xc = ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS);
       ...
       poly4(display,
        ...
        x + xpart(xc - ANGLE_4, xb), y + ypart(xc - ANGLE_4, xb),
        x + xpart(xc, xd) + xpart(xc - ANGLE_4, xb), y + ypart(xc, xd) + ypart(xc - ANGLE_4, xb),
        x + xpart(xc, xd) + xpart(xc + ANGLE_4, xb), y + ypart(xc, xd) + ypart(xc + ANGLE_4, xb),
        TRANS_B5_OUT);
        END_TRANS_MODE
      ...
      fcircle(ebullet[b].x_speed / GRAIN, ebullet[b].y_speed / GRAIN, 19 + grand(4), TCOL_B5);

      for (i = 0; i < ebullet[b].status; i ++)
      {
       xb = RLE_beam [ebullet[b].sprite_angle].x;
       yb = RLE_beam [ebullet[b].sprite_angle].y;
       draw_trans_rle_sprite(display, RLE_beam [ebullet[b].sprite_angle].sprite, x + xpart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 23 + 45 * i + xa) - xb, y + ypart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 23 + 45 * i + xa) - yb);
       fcircle(x + xpart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 45 * i + xa), y + ypart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 45 * i + xa), 5 + grand(2), TCOL_B5);
//       fcircle(x + xpart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 22 + 45 * i + xa), y + ypart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 22 + 45 * i + xa), 5, TCOL_B5);
     ...
     case EBULLET_BEAM:
      ...
      for (i = 0; i < ebullet[b].status; i ++)
      {
       ...
       yb = RLE_beam [ebullet[b].sprite_angle].y;
       draw_trans_rle_sprite(display, RLE_beam [ebullet[b].sprite_angle].sprite, x + xpart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 23 + 45 * i + xa) - xb, y + ypart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 23 + 45 * i + xa) - yb);
       fcircle(x + xpart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 45 * i + xa), y + ypart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 45 * i + xa), 5, TCOL_B5);
       fcircle(x + xpart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 22 + 45 * i + xa), y + ypart(ebullet[b].sprite_angle * (ANGLE_1 / SMALL_ROTATIONS), 22 + 45 * i + xa), 5, TCOL_B5);
      }
      break;
     case EBULLET_SHOT:
     ...
     if (ebullet[b].time < 20 && ebullet[b].new_ebullet == 1)
     {
      ...
      drawing_mode(DRAW_MODE_TRANS, NULL, 0, 0);
        poly4(display,
         ...
         x + xpart(xa + ANGLE_4 + ANGLE_8, ya), y + ypart(xa + ANGLE_4 + ANGLE_8, ya),
      ...
      drawing_mode(DRAW_MODE_SOLID, NULL, 0, 0);
       ...
       draw_trans_rle_sprite(display, dart_bullet [ebullet[b].sprite_angle] [ebullet[b].colour].sprite, x - xb, y - yb);
//  textprintf_ex(display, font, x, y, COL_WHITE, -1, "%i", ebullet[b].sprite_angle);
     ...
     if (ebullet[b].time < 20 && ebullet[b].new_ebullet == 1)
     {
      ...
      drawing_mode(DRAW_MODE_SOLID, NULL, 0, 0);
       ...
       draw_trans_rle_sprite(display, sdart_bullet [ebullet[b].sprite_angle] [ebullet[b].colour].sprite, x - xb, y - yb);
     if (ebullet[b].cord_wait > -1)
     {
      drawing_mode(DRAW_MODE_TRANS, NULL, 0, 0);
      pline(display, ebullet[b].cord_x / GRAIN, ebullet[b].cord_y / GRAIN,
     ...
     for (i = 0; i < 3; i ++)
     {
      xc = (int) (ebullet[b].angle2 + (i * ANGLE_3)) / (ANGLE_1 / SMALL_ROTATIONS);
     ...
     {
      TRANS_MODE
      circlefill(display, x, y, ebullet[b].time / 10, ebullet[b].cord_colour);
      END_TRANS_MODE
     }
      else
...
//       print_number(x, y, xc);
//       blit(display, screen, 0, 0, 0, 0, 640, 480);
        draw_trans_rle_sprite(display, diamond_bullet [xc] [ebullet[b].colour].sprite, x - xb, y - yb);
//       blit(display, screen, 0, 0, 0, 0, 640, 480);
      }
...
/*     case EBULLET_DOT:
     ...
     if (ebullet[b].cord_wait > -1)
     {
      ...
      x, y,
       TRANS_B5_OUT);
      circle(display, ebullet[b].cord_x / GRAIN, ebullet[b].cord_y / GRAIN, 1, TRANS_B5_OUT);
      drawing_mode(DRAW_MODE_SOLID, NULL, 0, 0);
     }
      break;
     ...
     if (ebullet[b].timeout < 40)
     {
      ...
      for (i = 0; i < 3; i ++)
      {
       draw_dart(x, y, ebullet[b].angle + (i * ANGLE_3) + ANGLE_6, xe, ye, ya, xc);
      }
...
//       draw_dart(x, y, (ANGLE_1 - (ebullet[b].angle + (i * ANGLE_3))) & 1023, xe, ye, xa, xd);
     ...
     if (ebullet[b].timeout < 40)
     {
      ...
      for (i = 0; i < 5; i ++)
      {
       draw_dart(x, y, ebullet[b].angle + (i * ANGLE_5) + ANGLE_8, xe, ye, ya, xc);
      }
...
//        draw_diamond(xe, ye, ebullet[b].angle2 + i * ANGLE_4, (ebullet[b].time * 12) / 37, (ebullet[b].time * 20) / 37, ebullet[b].cord_colour);
        draw_diamond(xe, ye, ebullet[b].angle2 + i * ANGLE_3, (ebullet[b].time * 9) / 37, (ebullet[b].time * 15) / 37, ebullet[b].cord_colour);
      }
       else
       {
      ...
      for (i = 0; i < 3; i ++)
      {
       draw_dart(x, y, ebullet[b].angle2 + (i * ANGLE_3) + ANGLE_6, xe, ye, TRANS_B1_OUT, EBCOL_B1);
      }
...
*/
     ...
     case EBULLET_STREAM:
      ...
      {
//        draw_diamond(xe, ye, ebullet[b].angle2 + i * ANGLE_4, (ebullet[b].time * 12) / 37, (ebullet[b].time * 20) / 37, ebullet[b].cord_colour);
        draw_diamond(xe, ye, ebullet[b].angle + ANGLE_4, (ebullet[b].time * 9) / 37, (ebullet[b].time * 15) / 37, ebullet[b].cord_colour);
      }
       else
       {
         draw_trans_rle_sprite(display, diamond_bullet [xc] [ebullet[b].colour].sprite, xe - xb, ye - yb);
...
void draw_dart(int x, int y, int angle, int out, int timeout, int cord_col, int col)
{
       ...
       if (timeout < 30)
       {
        ...
        draw_trans_rle_sprite(display, dart_bullet [xc] [col].sprite, xd - xb, yd - yb);
}
...
void draw_diamond(int x, int y, int angle, int out1, int out2, int cord_colour)
{
       TRANS_MODE
        poly4(display,
         ...
         x + xpart(angle + ANGLE_4, out1), y + ypart(angle + ANGLE_4, out1),
         x - xpart(angle, out2), y - ypart(angle, out2),
         x + xpart(angle - ANGLE_4, out1), y + ypart(angle - ANGLE_4, out1),
         cord_colour);
       END_TRANS_MODE
...
/*        if (xa < 1)
        ...
        xb = cloud[c].colour;
     ya = ;*/
     draw_trans_rle_sprite(display, RLE_ring [ya].sprite, x - RLE_ring [ya].x, y - RLE_ring [ya].y);
   break;
...
//      fcircle(x - xpart(pbullet[b].angle, 4), y - ypart(pbullet[b].angle, 4), 3 + grand(2), 7);
      fcircle(x - xpart(pbullet[b].angle, 6 + xa), y - ypart(pbullet[b].angle, 6 + xa), 3 + grand(2) + xa, TCOL_ORANGE);
      fcircle(x - xpart(pbullet[b].angle, 10 + xa), y - ypart(pbullet[b].angle, 9 + xa), 2 + grand(2) + xa, TCOL_ORANGE);
      break;
     case PBULLET_CANNON:
     case PBULLET_RECT:
...
void draw_a_cloud(int c)
{
 ...
 switch(cloud[c].type)
 {
    case CLOUD_SCIRCLE:
     TRANS_MODE
     circle(display, x, y, cloud[c].timeout, cloud[c].colour);
     END_TRANS_MODE
     break;
    case CLOUD_DELAY1:
     break;
    ...
    case CLOUD_PETAL3_DEBRIS:
     ...
     case CLOUD_PETAL1:
      ...
      angle = cloud[c].angle;
      poly4(display, x, y,
       x + xpart(angle + ANGLE_8 - ANGLE_32, xb), y + ypart(angle + ANGLE_8 - ANGLE_32, xb),
       x + xpart(angle, xa), y + ypart(angle, xa),
       x + xpart(angle - ANGLE_8 + ANGLE_32, xb), y + ypart(angle - ANGLE_8 + ANGLE_32, xb),
       TRANS_SH2_OUT);
      END_TRANS_MODE
      break;
     ...
     case CLOUD_SLINE:
      TRANS_MODE
      pline(display, x, y, cloud[c].x2 / GRAIN, cloud[c].y2 / GRAIN, cloud[c].colour);
...
//       circle(display, x, y, 1, TRANS_ORANGE_OUT);
     ...
     case CLOUD_RSHOCK5:
      ...
      draw_trans_rle_sprite(display, RLE_rshock [xb] [20 - xa].sprite, x - ya, y - ya);
      break;
     case CLOUD_MSHOCK:
        xa = cloud[c].timeout;
        if (xa < 1)
     ...
     draw_trans_rle_sprite(display, RLE_mshock [xb] [20 - xa].sprite, x - ya, y - ya);
   break;
     case CLOUD_LSHOCK:
        xa = cloud[c].timeout;
        if (xa < 1)
     ...
     draw_trans_rle_sprite(display, RLE_lshock [xb] [50 - xa].sprite, x - ya, y - ya);
   break;
     case CLOUD_HSHOCK:
        xa = cloud[c].timeout;
//        xa = 5 - (xa / 10);
     ...
     draw_trans_rle_sprite(display, RLE_hshock [xb] [50 - xa].sprite, x - ya, y - ya);
   break;
     ...
     case CLOUD_TSHOCK:
       ...
       {
        TRANS_MODE
        circle(display, x, y, xa + 2, TRANS_ORANGE_OUT);
        END_TRANS_MODE
        break;
       }
...
//        textprintf_ex(display, font, x, y + 20, COL_WHITE, -1, "%i", xa);
       ...
       for (i = 0; i < 4; i ++)
       {
         fcircle(x + xpart(angle, ya + 2), y + ypart(angle, ya + 2), (10 - xa) / 1, TCOL_ORANGE);
         angle += ANGLE_4;
       }
...
//     case CLOUD_SCIRCLE:
     ...
     case CLOUD_CARRIER1_BANG:
       ...
       for (i = 0; i < 6; i ++)
       {
         ...
         fcircle(x + xpart(angle, xa - 4), y + ypart(angle, xa - 4), ya / 2, TCOL_SH2);
         angle += ANGLE_6;
         angle &= 1023;
       }
...
// blit(underlay, display, 0, arena.underlay_position, 0, 0, 640, 480);
 ...
 for (i = 0; i < 20; i ++)
 {
       ...
       rectfill(display, arena.block2_x [i] - arena.block2_size [i], xb - arena.block2_size [i],
         arena.block2_x [i] + arena.block2_size [i], xb + arena.block2_size [i], COL_BACK1);
/*   drawing_mode(DRAW_MODE_TRANS, NULL, 0, 0);
...
//     circle(display, arena.block1_x [i], arena.block1_y [i], arena.block1_size [i], COL_OUTLINE);
       ...
       rectfill(display, arena.block1_x [i] - arena.block1_size [i], xb - arena.block1_size [i],
         arena.block1_x [i] + arena.block1_size [i], xb + arena.block1_size [i], COL_BACK2);
     drawing_mode(DRAW_MODE_TRANS, NULL, 0, 0);
       rect(display, arena.block1_x [i] - arena.block1_size [i] - 1, xb - arena.block1_size [i] - 1,
         arena.block1_x [i] + arena.block1_size [i] + 1, xb + arena.block1_size [i] + 1, TRANS_BACK3_OUTLINE);
     drawing_mode(DRAW_MODE_SOLID, NULL, 0, 0);
 }
...
/*           if (xb < 0)
           ...
           if (xb > 19)
            xb = 19;*/
           draw_rle_sprite(display, eRLE_flower [0] [xb].sprite, x - eRLE_flower [0] [xb].x + 42 + 3, y - eRLE_flower [0] [xb].y + 47);
          }
      }
 ...
 for (j = 0; j < 7; j ++)
 {
  ...
  for (i = 0; i < 9; i ++)
  {
      if (platform_tile [i] [across] != PLATFORM_EMPTY)
      {
          x = i * 91 - 82;
          y = j * 91 - 95 + platform_position;
         draw_rle_sprite(display, platform_RLE [platform_tile [i] [across]], x, y);
      }
  }
 }
...
*/
 ...
 for (j = 0; j < 7; j ++)
 {
  ...
  for (i = 0; i < 9; i ++)
  {
      if (island1_tile [i] [across] > ISLAND1_EMPTY && island1_tile [i] [across] < ISLAND1_RLES)
      {
          x = i * 91 - 82;
...
//  for (i = 0; i < 16; i ++)
  do
  {
   col = COL_BACK2;
   out = 0;
   cy = (-40 + (j * 40) + arena.part_cy [i] [p_rank] + arena.part_flow) * 1000;
   ...
   rectfill(display, 20 + (i * 40) + arena.part_x1 [i] [p_rank] - out,
                     ...
                     -40 + (j * 40) + arena.part_y2 [i] [p_rank] + arena.part_flow + out, col);
   rect(display, 20 + (i * 40) + arena.part_x1 [i] [p_rank] - out,
                     -40 + (j * 40) + arena.part_y1 [i] [p_rank] + arena.part_flow - out,
                     20 + (i * 40) + arena.part_x2 [i] [p_rank] + out,
                     -40 + (j * 40) + arena.part_y2 [i] [p_rank] + arena.part_flow + out, COL_OUTLINE);
...
//  circle(display, arena.bone_x [0] [i],    arena.bone_y [0] [i], 10, COL_WHITE);
  if (arena.bone_y [0] [i] > -50)
  {
       ...
       {
         case 0:
          draw_rle_sprite(display, bone_RLE [BONE_B_HOOK1 + arena.bone_subtype [0] [i]], arena.bone_x [0] [i] - 4, arena.bone_y [0] [i] - 4);
           break;
         case 1:
          draw_rle_sprite(display, bone_RLE [BONE_B_DROP1 + arena.bone_subtype [0] [i]], arena.bone_x [0] [i] - 5, arena.bone_y [0] [i] - 4);
           break;
         case 2:
          draw_rle_sprite(display, bone_RLE [BONE_B_RING1 + arena.bone_subtype [0] [i]], arena.bone_x [0] [i] - 11, arena.bone_y [0] [i] - 11);
           break;
         case 3:
          draw_rle_sprite(display, bone_RLE [BONE_B_HUNTER1 + arena.bone_subtype [0] [i]], arena.bone_x [0] [i] - 13, arena.bone_y [0] [i] - 11);
           break;
         case 4:
          draw_rle_sprite(display, bone_RLE [BONE_B_SPINNER], arena.bone_x [0] [i] - 32, arena.bone_y [0] [i] - 53);
           break;
         case 5:
          draw_rle_sprite(display, bone_RLE [BONE_B_WALKER1 + arena.bone_subtype [0] [i]], arena.bone_x [0] [i] - 44, arena.bone_y [0] [i] - 37);
           break;
       }
...
//  circle(display, arena.bone_x [0] [i],    arena.bone_y [0] [i], 10, COL_WHITE);
  if (arena.bone_y [1] [i] > -30)
  {
       ...
       {
         case 0:
          draw_rle_sprite(display, bone_RLE [BONE_HOOK1 + arena.bone_subtype [1] [i]], arena.bone_x [1] [i] - 11, arena.bone_y [1] [i] - 10);
           break;
         case 1:
          draw_rle_sprite(display, bone_RLE [BONE_DROP1 + arena.bone_subtype [1] [i]], arena.bone_x [1] [i] - 12, arena.bone_y [1] [i] - 17);
          break;
         case 2:
          draw_rle_sprite(display, bone_RLE [BONE_RING1 + arena.bone_subtype [1] [i]], arena.bone_x [1] [i] - 19, arena.bone_y [1] [i] - 19);
          break;
         case 3:
          draw_rle_sprite(display, bone_RLE [BONE_HUNTER1 + arena.bone_subtype [1] [i]], arena.bone_x [1] [i] - 22, arena.bone_y [1] [i] - 20);
          break;
...
void shift_message(void)
{
 ...
 switch(arena.level)
 {
  case 1:
   ...
   switch(arena.phase)
   {
     case 0: textprintf_ex(display, font, x, y, -1, -1, "YOU ARE BORN INTO SPRING"); break;
     case 1: textprintf_ex(display, font, x, y, -1, -1, "THE SUN IS HIGH ABOVE"); break;
     case 2: textprintf_ex(display, font, x, y, -1, -1, "AUTUMN IS THE WINDING DOWN"); break;
     case 3: textprintf_ex(display, font, x, y, -1, -1, "NO NEW LIFE WITHOUT DEATH"); break;
     case 4: textprintf_ex(display, font, x, y, -1, -1, "THE CYCLE IS RENEWED"); break;
   }
...
//       circle(display, 350, 441, x, COL_OUTLINE);
      ...
      if (x > 0)
      {
       circlefill(display, 350 + xpart(angle, 28), 441 + ypart(angle, 28), x, COL_COL4);
       circle(display, 350 + xpart(angle, 28), 441 + ypart(angle, 28), x, COL_OUTLINE);
      }
    }
  ...
  case 2:
   ...
   switch(arena.phase)
   {
     case 0: textprintf_ex(display, font, x, y, -1, -1, "IT IS COLD IN THE VOID"); break;
     case 1: textprintf_ex(display, font, x, y, -1, -1, "DISTANT STARS WARM THE DUST"); break;
     case 2: textprintf_ex(display, font, x, y, -1, -1, "DESTRUCTION BLOWS A HOT WIND"); break;
     case 3: textprintf_ex(display, font, x, y, -1, -1, "ORDER TURNS TO BOILING CHAOS"); break;
     case 4: textprintf_ex(display, font, x, y, -1, -1, "THE EQUILIBRIUM OF HEAT DEATH"); break;
   }
   if (arena.shift_message < 150)
   {
     x = 50 / 3;
     ...
     switch(arena.phase)
     {
      ...
      case 2:
       ...
       break;
      case 3:
       fcircle(340 + grand(3) - grand(3), 441 + grand(3) - grand(3), x, TCOL_ORANGE);
       fcircle(340 + grand(4) - grand(4), 441 + grand(4) - grand(4), x + 5 + grand(3), TCOL_SH2);
       y = (arena.shift_message / 2) % 8;
       circle(display, 340, 441, x - y + 8, TRANS_SH2_OUT);
       break;
      case 4:
       fcircle(340 + grand(4) - grand(4), 441 + grand(4) - grand(4), x, TCOL_YELLOW);
       fcircle(340 + grand(4) - grand(4), 441 + grand(4) - grand(4), x + 6 + grand(3), TCOL_ORANGE);
  ...
  case 3:
   ...
   switch(arena.phase)
   {
     case 0: textprintf_ex(display, font, x, y, -1, -1, "MOTION BENDS THE FABRIC OF SPACE"); break;
     case 1: textprintf_ex(display, font, x, y, -1, -1, "THE WAVES SEEM TO ACCELERATE"); break;
     case 2: textprintf_ex(display, font, x, y, -1, -1, "THE OUTER REGIONS CALL TO YOU"); break;
     case 3: textprintf_ex(display, font, x, y, -1, -1, "SPEED SHIFTS LIGHT TO BLUE"); break;
     case 4: textprintf_ex(display, font, x, y, -1, -1, "THERE IS NO PATH BACK FROM HERE"); break;
   }
   if (arena.shift_message < 200)
   {
     ...
     if (y <= 0)
      break;
     TRANS_MODE
     angle = arena.shift_message * ((1 + arena.phase * 9) * 4);
     angle &= 1023;
...
/*     angle = arena.shift_message * (5 + arena.phase);
  ...
  case 4:
   ...
   switch(arena.phase)
   {
     case 0: textprintf_ex(display, font, x, y, -1, -1, "THINGS COME HERE TO DIE"); break;
     case 1: textprintf_ex(display, font, x, y, -1, -1, "BONES FADE INTO THE GLOOM"); break;
     case 2: textprintf_ex(display, font, x, y, -1, -1, "CRUMBLING IN SILENCE"); break;
     case 3: textprintf_ex(display, font, x, y, -1, -1, "GONE THEN FORGOTTEN"); break;
     case 4: textprintf_ex(display, font, x, y, -1, -1, "DUST TO DUST"); break;
   }
    break;
...
void display_pause(int pc)
{
  ...
  textprintf_centre_ex(screen, font, 320, 150, -1, -1, "PRESS  'Y'  TO  QUIT");
  textprintf_centre_ex(screen, font, 320, 165, -1, -1, "OR  'N'  TO  CONTINUE");
